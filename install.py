...
rPackages = ["libcurl4", "libxslt1-dev", "libgeoip-dev", "libonig-dev", "e2fsprogs", "wget", "mcrypt", "nscd", "htop", "zip", "unzip", "mc", "mariadb-server", "libpng16-16", "libzip5", "python3-paramiko", "python-is-python3", "php7.4", "php7.4-fpm", "php7.4-cli", "php7.4-mysql", "php7.4-gd", "php7.4-xml", "php7.4-curl", "php7.4-zip", "php7.4-mbstring", "php7.4-json"]
...
def prepare(rType="MAIN"):
    global rPackages
    rType = rType.upper()  # Ensure uppercase for consistency
    if rType != "MAIN": rPackages = rPackages[:-10]  # Remove PHP packages for LB
    ...
    if rType == "MAIN":
        ...
        printc("Adding PHP 7.4 repository")
        os.system("add-apt-repository ppa:ondrej/php -y > /dev/null")
        os.system("apt-get update > /dev/null")
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            ...
            if input("  ").upper() == "Y":
                ...
                if rType == "MAIN":
                    ...
                    printc("ION Cube 7.4 installed successfully", col.BRIGHT_YELLOW)
                    ...
                    rFile.write("ION Cube 7.4 installed successfully\n")
                    rFile.close()
